...
使用 Read 工具读取 `modules/arch-init.md` 的内容，严格按照其中定义的步骤执行初始化流程。
...
使用 Read 工具读取 `modules/arch-engine.md` 的内容，执行智能经验加载部分。
...
使用 Read 工具读取 `modules/arch-generate.md` 的"Phase 1.5：需求分解"部分执行。
...
使用 Read 工具读取 `modules/arch-generate.md` 的"Phase 2：分层生成"部分执行。
...
评分时参考：使用 Read 工具读取 `modules/arch-engine.md` 获取维度→Agent→策略映射。

Agent 协作规范：使用 Read 工具读取 `modules/arch-agent-protocol.md` 获取 Agent 统一输出格式、协作规范。

---

## 八、阶段三：挑战循环

使用 Read 工具读取 `modules/arch-challenge.md` 的内容，严格按照其中定义的步骤执行循环。
...
高级引擎机制：使用 Read 工具读取 `modules/arch-engine.md` 获取能量管理、疲劳检测等高级机制。

Agent 协作协议：使用 Read 工具读取 `modules/arch-agent-protocol.md` 获取 Agent 统一输出格式、行为自适应等规范。

---

## 九、阶段四 + 阶段五：终审与交付

使用 Read 工具读取 `modules/arch-deliver.md` 的内容，严格按照其中定义的步骤执行。
...
| 文件 | 用途 | 加载时机 |
|------|------|---------|
| `modules/arch-init.md` | 初始化+约束收集+质量门槛 | 阶段零 |
| `modules/arch-generate.md` | 需求分解+分层生成 | 阶段 1.5+二 |
| `modules/arch-challenge.md` | 挑战循环+级联传播 | 阶段三 |
| `modules/arch-deliver.md` | 终审+交付 | 阶段四+五 |
| `modules/arch-engine.md` | 引擎机制（能量+信任度+疲劳+策略映射+经验） | 全程引用 |
| `modules/arch-agent-protocol.md` | 协作协议（输出格式+黑板+预检后检+级联矩阵） | 全程引用 |
